...
In our case we goin to simulate model locally so we will generate small number of parameter sets ($numSets=50$). We already split our original model into two part: parameter definition (listing~\ref{code:paramfile}) and variable part of the model (listing~\ref{code:varfile}). There is no constant part in our model, so we provide empty list into the $constantfiles$ parameter.
...
We going to change shell templates to be able to run it locally on average Linux or Mac system. There are three shell script templates in the project: 
\begin{description}
...
N<-att(tcs,'N')
p<-att(tcs,'p')
tset<-qt(c(1e-3,1e-2,5e-2),(N-2-p))
sqrt(tset^2/(tset^2+(N-2-p)))->gms
threshold<-t(data.frame(sig.level=c("0.1%","1%","5%"),sens.level=gms))
